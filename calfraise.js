...
function onResults(results) {
  ...
  if (results.poseLandmarks) {
    ...
    const rightFootIndex = landmarks[31];
    const rightAnkleAngle = calculateAngle(rightKnee, rightAnkle, rightFootIndex);

    // Left elbow angle
    const leftKnee = landmarks[26];
    const leftAnkle = landmarks[28];
    const leftFootIndex = landmarks[32];
    const leftAnkleAngle = calculateAngle(leftKnee, leftAnkle, leftFootIndex);
    ...
    if (leftAnkleAngle < 120) {
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
      canvasCtx.fillText('Stand on your toe', leftAnklePos.x, leftAnklePos.y - 10);
    }
    if (leftAnkleAngle > 140) {
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
      canvasCtx.fillText('Lower your heels', leftAnklePos.x, leftAnklePos.y - 10);
    }
    if (rightAnkleAngle < 120) {
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
      canvasCtx.fillText('Stand on your toe', rightAnklePos.x, rightAnklePos.y - 10);
    }
    if (rightAnkleAngle > 140) {
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
      canvasCtx.fillText('Lower your heels', rightAnklePos.x, rightAnklePos.y - 10);
    }
    else{
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
