...
const pushUpAudio = document.getElementById('pushUpAudio');
const pushDownAudio = document.getElementById('pushDownAudio');
const okAudio = document.getElementById('okAudio');

let lastAudioSrc = null;

function playAudio(audio) {
  // Use `play()` inside a try-catch and ensure it's not blocking
  try {
    if(!audio.paused && audio.src === lastAudioSrc){
      return;
    }
    ...
    if (playPromise !== undefined) {
      playPromise.catch(error => {
        ...
        // Optional: fallback UI or silent fail
      });
    }
  } catch (err) {
...
function onResults(results) {
  ...
  if (results.poseLandmarks) {
    ...
    if (leftElbowAngle < 80) {
      ...
      canvasCtx.fillText('Push Yourself Up', leftElbowPos.x, leftElbowPos.y - 10);
      playAudio(pushUpAudio);
    }
    if (leftElbowAngle > 160) {
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
      canvasCtx.fillText('Push Yourself Down', leftElbowPos.x, leftElbowPos.y - 10);
      playAudio(pushDownAudio);
    }
    if (rightElbowAngle < 80) {
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
      canvasCtx.fillText('Push Yourself Up', rightElbowPos.x, rightElbowPos.y - 10);
      // playAudio(pushUpAudio);
    }
    if (rightElbowAngle > 160) {
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
      canvasCtx.fillText('Push Yourself Down', rightElbowPos.x, rightElbowPos.y - 10);
      // playAudio(pushDownAudio);
    }
    else{
      canvasCtx.fillStyle = 'lime';
